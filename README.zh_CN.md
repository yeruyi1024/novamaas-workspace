...
- [镜像列表](https://github.com/yeruyi1024/novamaas-workspace/pkgs/container/novamaas-workspace)：PR 合并到 `main` 后由 GitHub 托管 runner 发布 `build_<UTC 合并时间>_amd64`、`build_<UTC 合并时间>_arm64` 和 `build_<UTC 合并时间>_multiarch`；self-hosted runner 独立构建 Linux amd64 镜像，并以相同时间戳的 `build_<UTC 合并时间>_amd64` 推送至 `ccr.ccs.tencentyun.com/nova-proj/nova-maas`。
