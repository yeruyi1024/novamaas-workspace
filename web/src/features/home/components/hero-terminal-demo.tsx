...
export function HeroTerminalDemo(props: HeroTerminalDemoProps) {
  ...
  return (
    <div
      className={cn(
        'maas-terminal mx-auto w-full min-w-0 max-w-2xl',
        props.className
      )}
    >
      <div className='mb-4 flex items-center gap-3 text-xs'>
        <span className='maas-accent font-semibold'>{t('API preview')}</span>
      ...
      >
        ...
        <div
          className={cn(
            'flex flex-wrap items-center gap-1 border-b px-2 sm:gap-1.5 sm:px-3',
        ...
        <div className='grid h-[400px] grid-rows-[235px_minmax(0,1fr)] font-mono text-[12.5px] leading-[1.55]'>
          {/* Request */}
          <RequestBlock demo={demo} />

          {/* Response */}
          <ResponseBlock demo={demo} />
        </div>

        <div className='border-border/40 bg-muted/20 border-t px-5 py-3'>
...
function RequestBlock(props: { demo: ApiDemoConfig }) {
  const { demo } = props
  const { t } = useTranslation()

  return (
    <div
      className='relative min-h-0 overflow-auto px-5 py-4'
      tabIndex={0}
      role='region'
...
function ResponseBlock(props: { demo: ApiDemoConfig }) {
  const { demo } = props
  const { t } = useTranslation()
  ...
  return (
    <div
      ...
      className={cn(
        'relative min-h-0 overflow-auto border-t px-5 py-4',
