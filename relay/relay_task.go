...
func videoFetchByIDRespBodyBuilder(c *gin.Context) (respBody []byte, taskResp *dto.TaskError) {
	...
	// Gemini/Vertex/Ali 支持实时查询：用户 fetch 时直接从上游拉取最新状态
...
// tryRealtimeFetch 尝试从上游实时拉取 Gemini/Vertex/Ali 任务状态。
// 仅当渠道类型支持实时查询时触发；其他渠道或出错时返回 nil。
...
func tryRealtimeFetch(task *model.Task, isOpenAIVideoAPI bool) []byte {
	...
	if key == "" {
		key, _, _ = channelModel.GetNextEnabledKey()
	}
	if key == "" {
		return nil
	}
	...
	// A dashboard read must not win the terminal-state CAS because the polling
	// service owns settlement/refund when a task first reaches a terminal state.
	// The fresh terminal result is still returned from this in-memory snapshot.
	isNewTerminalState := (task.Status == model.TaskStatusSuccess || task.Status == model.TaskStatusFailure) &&
		snap.Status != task.Status
	if !isNewTerminalState && !snap.Equal(task.Snapshot()) {
