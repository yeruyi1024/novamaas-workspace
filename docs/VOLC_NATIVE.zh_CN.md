...
- 模型：填写对客户端公开的模型名称；不使用模型映射时，直接填写火山方舟的上游模型 ID

该渠道支持模型映射。例如可在“模型”中公开 `seedance-pro`，再配置 `{"seedance-pro":"doubao-seedance-2-0-260128"}`。平台使用公开名称进行渠道选择、令牌权限检查、计费和日志记录，仅在发往火山方舟前把请求顶层的 `model` 字段改为上游模型 ID。映射别名需要单独配置价格或倍率，不会自动继承目标模型的价格。

参数覆盖仍不受支持；如需修改其他原生字段，应由客户端直接在请求体中传入。
...
请求字段直接放在 JSON 顶层，**不需要增加 `metadata` 包装层**。服务端仅在内部读取顶层字段完成校验和计费估算。未命中模型映射时，发往火山方舟的 JSON 字节保持原样；命中映射时，仅改写顶层 `model` 字段，其余原生字段和值保持不变。
...
- 火山原生与 DoubaoVideo 渠道均可按渠道及模型开启 Base64 媒体暂存。暂存会处理 `content[].image_url.url` 中的 JPEG、PNG、WebP Data URI，以及 `content[].video_url.url` 中的 MP4、WebM、MOV Data URI；校验文件特征后上传到私有对象存储，并仅在上游请求副本中替换为限时签名地址。
- DoubaoVideo 先把兼容接口的 `metadata.content` 转换为火山原生 `content`，再执行暂存和顶层模型映射，因此图片、视频与映射模型可以同时使用；重试会复用同一批暂存结果，避免重复上传。
